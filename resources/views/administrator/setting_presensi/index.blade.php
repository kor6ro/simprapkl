...
@section('content')
    ...
    <div class="row mb-3">
        <div class="col-auto">
            <a href="{{ route('setting_presensi.create') }}" class="btn btn-success">
    ...
    <div class="d-none">
        <form id="form-destroy" action="{{ route('setting_presensi.store') }}" method="post">
            @csrf
            @method('DELETE')
        </form>
    </div>
...
@section('js')
    <script>
        $('table').DataTable({
            ...
            autoWidth: false,
            ajax: {
                url: baseUrl('/setting_presensi/fetch'),
                headers: {
            ...
            createdRow: function(row, data) {
                $(".action-edit", row).click(function(e) {
                    const url = baseUrl('/setting_presensi/' + data.id + '/edit');
