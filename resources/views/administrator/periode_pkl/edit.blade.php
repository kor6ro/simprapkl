...
@section('content')
    <div class="row">
        <div class="col-12">
            <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                <h4 class="mb-sm-0 font-size-18">Edit Periode PKL</h4>
    ...
    <div class="card card-primary">
        <div class="card-body">
            <form action="{{ route('admin.periode-pkl.update', $periodePkl->id) }}" method="post">
                @csrf
                @method('PUT')
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <input class="form-control" type="date" name="awal_periode" id="awal_periode"
                            value="{{ old('awal_periode', $periodePkl->awal_periode->format('Y-m-d')) }}" required>
                        @error('awal_periode')
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input class="form-control" type="date" name="akhir_periode" id="akhir_periode"
                            value="{{ old('akhir_periode', $periodePkl->akhir_periode->format('Y-m-d')) }}" required>
                        @error('akhir_periode')
                            <small class="text-danger">{{ $message }}</small>
                        @enderror
                    </div>
...
@section('js')
    ...
    <script>
        // Inisialisasi Tom-Select untuk Siswa dengan data yang sudah ada
        new TomSelect('#select-siswa', {
            plugins: ['remove_button'],
            placeholder: 'Pilih satu atau lebih siswa...',
            // Memuat pilihan lama jika ada, jika tidak, muat data dari database
            items: @json(old('siswa_ids', $selectedSiswaIds))
        });

        // Inisialisasi Tom-Select untuk Pembimbing dengan data yang sudah ada
        new TomSelect('#select-pembimbing', {
            plugins: ['remove_button'],
            placeholder: 'Pilih satu atau lebih pembimbing...',
            items: @json(old('pembimbing_ids', $selectedPembimbingIds))
        });
    </script>
@endsection
