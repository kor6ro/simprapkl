...
@section('content')
    ...
    <div class="row mb-3">
        <div class="col-auto">
            <a href="{{ route('task_breakdown.create') }}" class="btn btn-success">
    ...
    <div class="d-none">
        <form id="form-destroy" action="{{ route('task_breakdown.store') }}" method="post">
            @csrf
            @method('DELETE')
        </form>
    </div>
...
@section('js')
    <script>
        $('table').DataTable({
            ...
            autoWidth: false,
            ajax: {
                url: baseUrl('/task_breakdown/fetch'),
                headers: {
            ...
            createdRow: function(row, data) {
                $(".action-edit", row).click(function(e) {
                    const url = baseUrl('/task_breakdown/' + data.id + '/edit');
